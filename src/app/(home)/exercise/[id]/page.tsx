...
const Exercise = ({ params }: { params: { id: string } }) => {
  ...
  async function compileCode() {
    try {
      const compileOutput = await compileExerciseCode(
        userExercise?.id,
        exerciseCodeResult,
      );
        ...
        <div className="flex gap-16 mb-8">
              ...
              <div className="flex flex-col ml-8">
                <div className="flex items-center">
                  <span className="font-semibold text-base text-[#F0B40D]">
                    + {userExercise?.exercise.coinsToWin}
                  </span>
                  <div className="w-7">
                ...
                <span className="font-medium text-base text-light-gray">
                  + {userExercise?.exercise.xpToWin}xp
                </span>
              </div>
            </div>
          </div>
          <div className="flex flex-col gap-4 py-4">
            ...
            <div className="flex gap-8">
              <Technology
                technology={userExercise?.exercise.technologies}
                xpPoints={userExercise?.exercise.xpToWin}
              />
            </div>
          </div>
        </div>
